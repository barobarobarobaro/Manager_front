...
export default function LoginPage() {
  ...
  const handleSocialLogin = (provider) => {
    // 실제 구현에서는 각 소셜 로그인 API를 호출하고 사용자 정보를 확인
    console.log(`${provider} 로그인 시도`);
    
    // 예시: 소셜 로그인 후 회원 여부 확인
  ...
  // 사용자 존재 여부 확인 (실제 구현에서는 API 호출)
  const checkUserExists = async (provider) => {
    // 임시 구현: 실제로는 백엔드 API를 호출하여 확인
    return new Promise(resolve => {
      setTimeout(() => {
        // 테스트용 데이터
        resolve(true); // 테스트를 위해 항상 true 반환
      }, 500);
    });
  };
      ...
      <div className="w-full max-w-sm bg-white p-8 rounded-lg shadow-lg">
        ...
        {isNewUser ? (
          <div className="mb-6 p-4 bg-yellow-50 rounded-md">
            <p className="text-center text-gray-700">처음 방문하셨네요! 간단한 추가 정보를 입력하시면 서비스 이용이 가능합니다.</p>
            <button
